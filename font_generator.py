...
def save_description(letter: str, noise_level: int):
    # if letter  can be a number, change letter name to number
    if letter.isdigit():
        label = f"number {letter}"
    else:
        label = f"letter {letter}" 
    return f"{letter}.png:{label}, noise_level={noise_level}%\n"
...
def generate_font_image(width: int, height: int, x_position: int,
    ...
    assert noise_level >= 0 and noise_level <= 255, "Noise level must be between 0 and 255"
    ...
    description = save_description(letter, noise_level)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--letter", type=str, help="Letter to generate", required=True)
    ...
    generate_font_image(args.width, args.height, args.x_position, args.y_position, args.font_file, args.letter, args.noise_level, args.output_dir, args.overwrite)