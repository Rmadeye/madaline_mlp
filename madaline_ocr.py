...
class Network:
    ...
    def predict(self, test_set):
        ...
        for label, x in test_data.items():
            ...
            print(
                f"Letter: {label} -> {best_neuron.labels}, fidelity: {round(confidence[0][0],3)}, noise level: {noise_levels[label]}%"
            )
        total_accuracy = sum(
            [True for y, y_pred in result.items() if y == y_pred[0]]
        ) / len(result)
    ...
    def plot_cm(self, result_dict: dict, total_accuracy: float, noise_levels: float):
        ...
        ax.set_title(
            f"Confusion Matrix, total accuracy: {total_accuracy:.2%}", fontsize=30
        )
